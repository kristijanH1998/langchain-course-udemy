...
def implement_execute_tool_call(tool_call: Dict, available_tools: List[Tool]) -> str:
    ...
    if tool_to_use is None:
        raise ValueError(f"Tool with name {tool_name} is not available")
    result = ""
    if "text" in tool_args:
        result = tool_to_use.invoke(tool_args['text'])
    return str(result)
    pass
...
def implement_run_agent_with_tool_calling(model_with_tools: ChatOpenAI, 
    ...
    if len(ai_message.tool_calls) > 0:
        result = implement_execute_tool_call(ai_message.tool_calls[0], available_tools)
        return result
    else:
